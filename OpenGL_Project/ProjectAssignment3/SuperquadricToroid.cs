...
namespace OpenGL_Project.ProjectAssignment3
{
    class SuperquadricToroid : GeometricObject
    {
        ...
        public float OffsetParam;

        public NormalPackage Normals;

        public SuperquadricToroid()
        {
        ...
        public void GenerateCoords()
        {
            ...
            for (float theta = 0.0f; theta < 2 * Math.PI; theta += angleDiff)
            {
                ...
                {
                    coord = new Coordinate((float)(XCompParam * (OffsetParam + Math.Pow(Math.Cos(phi), PhiResolParam)) * Math.Pow(Math.Cos(theta), ThetaResolParam))
                        , (float)(YCompParam * (OffsetParam + Math.Pow(Math.Cos(phi), PhiResolParam)) * Math.Pow(Math.Sin(theta), ThetaResolParam))
                        , (float)(ZCompParam * Math.Pow(Math.Sin(phi), PhiResolParam)));
                    normal = new Vector((float)(Math.Pow(Math.Cos(phi), 2 - PhiResolParam) * Math.Pow(Math.Cos(theta), 2 - ThetaResolParam) / XCompParam)
                        , (float)(Math.Pow(Math.Cos(phi), 2 - PhiResolParam) * Math.Pow(Math.Sin(theta), 2 - ThetaResolParam) / YCompParam)
                        , (float)(Math.Pow(Math.Sin(phi), 2 - PhiResolParam) / ZCompParam));
                    coords.Add(coord);
